...
output "affinity" {
  value = var.create ? one(concat(
    google_compute_backend_service.default.*.session_affinity,
    google_compute_region_backend_service.default.*.session_affinity,
  )) : null
}
...
output "bucket_name" { value = local.is_bucket ? var.params.bucket_name : null }
